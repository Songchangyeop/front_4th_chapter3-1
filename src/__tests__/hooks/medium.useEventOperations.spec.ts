...
it("새로 정의된 'title', 'endTime' 기준으로 적절하게 일정이 업데이트 된다", async () => {
  ...
  act(() => useEventFormStore.setState({ editingEvent: newEvent }));
  ...
  await waitFor(() => {
    expect(result.current.events).toEqual([initialEvent[0], newEvent]);
...
it("존재하지 않는 이벤트 수정 시 '일정 저장 실패'라는 토스트가 노출되며 에러 처리가 되어야 한다", async () => {
  server.use(
    http.get('/api/events', () => {
      return HttpResponse.json({ message: '존재하지 않는 이벤트' }, { status: 404 });
    })
  );

  const { result } = renderHook(() => useEventOperations());

  await result.current.saveEvent(initialEvents[10]);
...
it("네트워크 오류 시 이벤트 삭제를 하면 '일정 삭제 실패'라는 텍스트가 노출되며 이벤트 삭제가 실패해야 한다", async () => {
  server.use(
